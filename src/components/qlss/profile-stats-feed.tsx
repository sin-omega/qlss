...
/**
 * CLI-styled flat list of recent profile visitors. Each entry is a compact,
 * monospaced record — same format as the link analytics feed.
 */
export function ProfileStatsFeed({ rows }: { rows: ProfileViewRow[] }) {
  return (
...
function VisitorRow({ row }: { row: ProfileViewRow }) {
  const location = formatLocation(row.country, row.region);
  const when = formatWhen(row.viewed_at, row.timezone);
...
function formatReferer(referer: string | null): string | null {
  ...
  // If it's a short label (like "profile" from ref=profile), return as-is
  if (!referer.includes("/")) return referer;
  try {
