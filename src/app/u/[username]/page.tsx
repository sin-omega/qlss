...
import { ArrowLeft, ExternalLink, MapPin, Clock, Globe } from "lucide-react";
 ...
 * Modern card-based design with glassmorphism, gradient accents, and
 * platform-specific social icons. Takes inspiration from Linktree,
 * Bento, and Carrd — but keeps the QLSS monospace DNA.
 */
export default async function UserProfilePage({
  ...
  const borderRadiusPx =
    borderRadiusSetting === "pill" ? 999 : borderRadiusSetting === "rounded" ? 12 : 0;
  ...
  // Build profile referrer query param for link clicks
  const refParam = "ref=profile";
  ...
  // Compute relative join date
  const joinDate = new Date(profile.created_at);
  const now = new Date();
  const daysSinceJoin = Math.floor((now.getTime() - joinDate.getTime()) / (1000 * 60 * 60 * 24));
  const memberSince = daysSinceJoin < 1 ? "today" : daysSinceJoin === 1 ? "yesterday" : `${daysSinceJoin}d ago`;

  function linkHref(slug: string): string {
    return `/${slug}?${refParam}`;
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-lg mx-auto px-5 py-8">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-[11px] mb-6 opacity-60 hover:opacity-100 transition-opacity"
          style={{ color: fgColor }}
        >
        ...
        {/* === Profile Card === */}
        <div className="rounded-2xl overflow-hidden" style={{ borderRadius: borderRadiusPx }}>
          {/* Banner */}
          <div
            className="relative h-32 sm:h-40 overflow-hidden"
            style={{
              backgroundColor: bannerUrl ? undefined : `${accentColor}18`,
          ...
          >
            ...
            {bannerUrl && (
              <div
                ...
                style={{
                  background: `linear-gradient(to top, ${cardColor} 0%, transparent 60%)`,
                }}
              />
            )}
            {!bannerUrl && (
              <div
                className="absolute inset-0"
                style={{
                  background: `radial-gradient(ellipse at 50% 120%, ${accentColor}20 0%, transparent 70%)`,
                }}
              />
            )}
          </div>
          ...
          >
            {/* Avatar */}
            <div
              className="inline-flex items-center justify-center text-4xl sm:text-5xl mb-3"
              style={{
                width: 72,
                height: 72,
                borderRadius: borderRadiusPx,
            ...
            >
              {avatarEmoji || (
                <span style={{ color: accentColor, fontFamily, fontWeight: 700, fontSize: "1.5rem" }}>
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              ...
              <span className="text-[11px] inline-flex items-center gap-1" style={{ color: mutedColor }}>
                <Clock className="h-3 w-3" />
                joined {memberSince}
              </span>
              {statusText && (
                <span
                  className="text-[11px] font-medium inline-flex items-center gap-1 px-2 py-0.5"
                  style={{
                    color: accentColor,
                    backgroundColor: `${accentColor}12`,
                    borderRadius: borderRadiusPx > 12 ? 999 : borderRadiusPx,
                  }}
                >
                  {statusText}
                </span>
            ...
            {socialLinks.length > 0 && (
              <div className="flex items-center gap-2.5 mt-4 flex-wrap">
                {socialLinks.map((sl, i) => (
                  ...
                  >
                    <SocialIconSvg platform={sl.platform} size={18} />
                  </a>
                ))}
              </div>
            )}
        ...
        {ctaText && ctaUrl && (
          <div className="mt-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full text-center text-sm font-semibold py-3 px-6 transition-all hover:brightness-110 active:scale-[0.98]"
              style={{
        ...
        <section className="mt-6">
          {links.length > 0 && (
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-[11px] uppercase tracking-widest font-medium" style={{ color: mutedColor }}>
                links
              </h2>
          ...
          {links.length === 0 ? (
              ...
              {links.map((link) => (
                <li key={link.id}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block p-3.5 hover:scale-[1.02] active:scale-[0.98]"
                    style={cardStyles()}
                  >
                    ...
                    {showDestination && (
                      <p className="text-[10px] mt-1 truncate" style={{ color: mutedColor, opacity: 0.5 }}>
            ...
            <ul className={layout === "compact" ? "space-y-1" : "space-y-2.5"}>
              {links.map((link) => (
                <li key={link.id}>
                  <a
                    href={linkHref(link.slug)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`block p-3.5 hover:scale-[1.01] active:scale-[0.98] ${layout === "compact" ? "p-2.5" : ""}`}
                    style={cardStyles()}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <ExternalLink className="h-3.5 w-3.5 shrink-0" style={{ color: mutedColor, opacity: 0.5 }} />
                    </div>
                  </a>
                </li>
              ))}
        ...
        <div className="mt-10 mb-4 text-center">
          <p className="text-[10px]" style={{ color: `${mutedColor}66` }}>
            powered by{" "}
            <a
              href="/"
...
/**
 * Server-rendered SVG social icons with configurable size.
 */
function SocialIconSvg({ platform, size = 24 }: { platform: string; size?: number }) {
