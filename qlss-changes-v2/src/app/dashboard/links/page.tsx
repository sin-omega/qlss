...
export default async function DashboardLinksPage({
  ...
  // Fetch the user's folders. Try with profile_page, fall back without.
  let folders: Array<Record<string, unknown>> | null = null;
  let foldersError: { message: string } | null = null;
  try {
    const result = await supabase
      .from("folders")
      .select("id, name, created_at, profile_page")
      .order("created_at", { ascending: true });
    folders = result.data as Array<Record<string, unknown>> | null;
    foldersError = result.error;
  } catch {
    folders = null;
    foldersError = { message: "query failed" };
  }

  if (foldersError && foldersError.message.includes("profile_page")) {
    const fallback = await supabase
      .from("folders")
      .select("id, name, created_at")
      .order("created_at", { ascending: true });
    folders = fallback.data as Array<Record<string, unknown>> | null;
